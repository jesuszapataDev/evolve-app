...
CREATE TRIGGER `trg_security_questions_delete` BEFORE DELETE ON `security_questions` FOR EACH ROW BEGIN
  INSERT INTO audit_log (
    ...
    'security_questions', OLD.security_question_id, 'DELETE_PHYSICAL', @user_id,
    @full_name, @user_type, NOW(), @action_timezone,
    NULL,
    JSON_OBJECT(
...
CREATE TRIGGER `trg_security_questions_delete_logical` AFTER UPDATE ON `security_questions` FOR EACH ROW BEGIN
  IF NEW.deleted_at IS NOT NULL AND OLD.deleted_at IS NULL THEN
    INSERT INTO audit_log (
      ...
      'security_questions', OLD.security_question_id, 'DELETE_LOGICAL', @user_id,
      @full_name, @user_type, NOW(), @action_timezone,
      JSON_OBJECT('deleted_at', JSON_OBJECT('old', NULL, 'new', NEW.deleted_at)),
      JSON_OBJECT(
...
CREATE TRIGGER `trg_security_questions_insert` AFTER INSERT ON `security_questions` FOR EACH ROW BEGIN
  INSERT INTO audit_log (
    ...
    'security_questions', NEW.security_question_id, 'INSERT', @user_id,
    @full_name, @user_type, NOW(), @action_timezone,
    NULL,
    JSON_OBJECT(
...
CREATE TRIGGER `trg_security_questions_update` AFTER UPDATE ON `security_questions` FOR EACH ROW BEGIN
  ...
  IF OLD.user_id_user <> NEW.user_id_user THEN
    SET change_data = CONCAT(change_data, '"user_id_user":{"old":"', OLD.user_id_user, '","new":"', NEW.user_id_user, '"},');
  END IF;
  IF OLD.user_id_admin <> NEW.user_id_admin THEN
    SET change_data = CONCAT(change_data, '"user_id_admin":{"old":"', OLD.user_id_admin, '","new":"', NEW.user_id_admin, '"},');
  END IF;
  IF OLD.user_id_specialist <> NEW.user_id_specialist THEN
    SET change_data = CONCAT(change_data, '"user_id_specialist":{"old":"', OLD.user_id_specialist, '","new":"', NEW.user_id_specialist, '"},');
  END IF;
  IF OLD.user_type <> NEW.user_type THEN
    SET change_data = CONCAT(change_data, '"user_type":{"old":"', OLD.user_type, '","new":"', NEW.user_type, '"},');
  END IF;
  IF OLD.question1 <> NEW.question1 THEN
    SET change_data = CONCAT(change_data, '"question1":{"old":"', OLD.question1, '","new":"', NEW.question1, '"},');
  END IF;
  IF OLD.answer1 <> NEW.answer1 THEN
    SET change_data = CONCAT(change_data, '"answer1":{"old":"', escape_json(OLD.answer1), '","new":"', escape_json(NEW.answer1), '"},');
  END IF;
  IF OLD.question2 <> NEW.question2 THEN
    SET change_data = CONCAT(change_data, '"question2":{"old":"', OLD.question2, '","new":"', NEW.question2, '"},');
  END IF;
  IF OLD.answer2 <> NEW.answer2 THEN
    SET change_data = CONCAT(change_data, '"answer2":{"old":"', escape_json(OLD.answer2), '","new":"', escape_json(NEW.answer2), '"},');
  END IF;
  ...
  IF change_data <> '{}' THEN
    INSERT INTO audit_log (
      ...
      'security_questions', OLD.security_question_id, 'UPDATE', @user_id,
      @full_name, @user_type, NOW(), @action_timezone,
      change_data, NULL,
      @client_ip, @client_hostname, @user_agent,
      @client_os, @client_browser,
